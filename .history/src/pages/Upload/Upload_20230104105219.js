...
import { ToggleOff, ToggleOn } from '~/components/Icon/Icon';
...
function Upload() {
  ...
  const [isClickDuet, setIsClickDuet] = useState(false);
  const [isClickStich, setIsClickStich] = useState(false);
  ...
  const handleClickComment = () => {
    // setIsChecked(true);
    setIsClickComment((current) => !current);
  };
  const handleClickDuet = () => {
    // setIsChecked(true);
    setIsClickDuet((current) => !current);
  };
  const handleClickStich = () => {
    // setIsChecked(true);
    setIsClickStich((current) => !current);
  };
              ...
              <div className={cx('item-left')}>
                <div className={cx('content-manipulation-upload')}>
                    ...
                    <div className={cx('check-user')}>
                      <div className={cx('checkbox-user')}>
                        <input
                          type="checkbox"
                          value="comment"
                          className={cx('inp-check-user')}
                          checked={isClickComment}
                        />
                        <span onClick={handleClickComment}></span>
                        Comment
                      </div>
                      <div className={cx('checkbox-user')}>
                        <input type="checkbox" value="Duet" className={cx('inp-check-user')} checked={isClickDuet} />
                        <span onClick={handleClickDuet}></span>
                        Duet
                      </div>
                      <div className={cx('checkbox-user')}>
                        <input type="checkbox" value="Stich" className={cx('inp-check-user')} checked={isClickStich} />
                        <span onClick={handleClickStich}></span>
                        Stich
                      </div>
                    </div>
                  </div>
                  <div className={cx('finish-check')}>
                    ...
                    {/* <FontAwesomeIcon className={cx('toggle-check')} icon={faToggleOn} /> */}
                    <div className={cx('icon-check')} onChange={handleToggle}>
                      <ToggleOff className={cx('icon-tgoff')} style={{ display: isToggle ? 'none' : '' }} />
                      <ToggleOn className={cx('icon-tgon')} style={{ display: isToggle ? '' : 'block' }} />
                    </div>
                  </div>
                  ...
                  <div className={cx('btn-submit')}>
                    ...
                    <div className={cx('btn-post')} disible>
